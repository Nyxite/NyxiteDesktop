...
- **Post-quantum crypto — hybrid at v1.0.0 (RESOLVED, [OPEN-DECISIONS PQ-1–PQ-4](https://github.com/Nyxite/Nyxite), 2026-07-07)**. Not an open question: every asymmetric seam ships **hybrid classical + PQC** from v1.0.0 — **X25519 + ML-KEM-768** HPKE key wrap / key agreement and **Ed25519 + ML-DSA-65** signatures, both **NIST level 3**; symmetric primitives (AES-256-GCM, Argon2id, BLAKE3) are unchanged ([06 §6.2](06-cryptography.md)). The desktop consumes whatever hybrid-capable `Nyxite.Crypto` the server ships; the **one open engineering item** is confirming a hybrid-capable HPKE / signature stack for the shared package — a **server-owned library choice**, tracked as the follow-up in [02 §2.7](02-tech-stack-and-libraries.md), not decided here.
- **Password pepper (RESOLVED, [OPEN-DECISIONS "password pepper"](https://github.com/Nyxite/Nyxite), 2026-07-07)**. The account-password verifier is now **Argon2id over HMAC-SHA256(password, pepper)** with a **server-held** pepper. **No desktop change**: the client still sends the plain password over TLS and never holds or applies the pepper; the login password still never feeds content-key derivation ([14 §14.1](14-authentication.md)).
