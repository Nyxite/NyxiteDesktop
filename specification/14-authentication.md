...
- **Password + TOTP**: the app collects the password and the TOTP code and posts them to the server's native auth endpoint over TLS. The server verifies the Argon2id password verifier and the TOTP itself (the app does not implement TOTP) and returns **access + refresh tokens**.
...
**Enterprise option — Keycloak OIDC (Authorization Code + PKCE).** When an instance is configured for enterprise SSO, the app instead runs the OIDC flow, which resolves to the **same internal server token**:
