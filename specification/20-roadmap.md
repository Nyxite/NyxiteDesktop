...
## Phase 4.4 — Enterprise/family groups (group-key sharing)
Build-phase **4.4 in v1.0.0**, landing **after Phase 4.3 (key transparency)** and before the v1.0.0 release closer, mirroring the master build plan ([implementation/phase-4.4-groups.md](https://github.com/Nyxite/Nyxite); feature [groups.md](https://github.com/Nyxite/Nyxite)). Group enrollment **depends on** the key-transparency **inclusion-proof** check, which is pulled into v1.0.0 as **Phase 4.3** for this reason (the broader safety-number verification UI for one-off directory shares stays the optional Phase-6 item, below — 4.3 provides the log + proof groups need).

**Deliver** (steps **P4.4-DSK-1/2**): the **group-key layer** `personal key → group key → DEK → file` via the shared `Nyxite.Crypto` — group keypair generation, HPKE wrap/unwrap of the group key and DEK-to-group with the `alg_id`-tagged format ([06 §6.10](06-cryptography.md)); **member enrollment** verified against the key-transparency log (Phase 4.3), append-only group-key **grant** handling, and the **`GroupKeyRotationService`** for scope-scoped rotation on member removal (`409`/`412`, optional DEK re-seal) ([07 §7.10](07-key-and-device-management.md)); **group-management UI**, wrapping a file/subtree to a group, the per-project/folder **reader-group attachment** + **auto-wrap on create** (enterprise "manager reads all"), and **honest revocation UI** ([13 §13.7](13-sharing.md), [16 §16.8](16-offline-and-storage-policies.md)); recovery restoring group access automatically ([07 §7.10](07-key-and-device-management.md)).
**Done when**: a family group reads a shared folder (one grant per member, no file duplication) and an enterprise reader-group auto-wraps a worker's new files to the managers group (manager reads, other worker cannot); enrollment is transparency-checked; removing a member soft-deletes the grant and a remaining member rotates the affected scope's group key; recovery restores group access with no special step — the server holding only opaque grants/DEK-to-group wraps/membership rows throughout.
