...
| Backup/cloud-sync exfiltration | Store under app-private, non-roaming paths; document excluding the data dir from OS/file-sync backups; never write plaintext to an **external** path except an explicit user export ([16 §16.7](16-offline-and-storage-policies.md)). *(A `plaintext` scope of the opt-in plaintext-at-rest tri-state — [16 §16.2a](16-offline-and-storage-policies.md), SPECIFICATION §6/§15 — does keep plaintext **inside** the app-private store; that is the user-accepted disk-theft trade-off, still excluded from backups.)* |
