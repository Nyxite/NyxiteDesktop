...
**Verdict: NO.** The desktop `specification/` is unusually complete and internally consistent — for the large majority of surfaces (architecture, data model, crypto framing, sync engine, collaboration, editors, search, version history, sharing, auth, UI, security, packaging) there is more than enough to build against. But there are **6 hard blockers** where required information is either **missing or explicitly still open**, plus several minor ambiguities. Five blockers are concentrated at the seams with the *server* repo (whose source/packages are a third repo not provided here — and which is itself spec-only, so the shared packages do not exist as code yet); the sixth is the one **desktop-owned** gap, where the tri-state plaintext-at-rest cascade is now ratified but its implementation mechanics remain undefined.
...
### HB-1 — Plaintext-at-rest (desktop-only, tri-state): the spec contradiction is resolved, but the implementation mechanics are still open
...
**Status update — the spec defect is fixed:** The prior contradiction (the desktop `specification/` asserting an "always encrypted at rest" invariant against the global spec/plan's tri-state requirement) has been **resolved**. A new **`specification/16 §16.2a`** now defines the ratified `inherit`/`plaintext`/`encrypted` cascade, and this feature remains a first-class, ratified requirement (`FEATURES.md`, `Nyxite/features/desktop.md` Sync section, `OPEN-DECISIONS.md` **#10**, `SPECIFICATION.md` §6/§15, and `phase-1.2-storage-control-and-search.md` deliverable + `P1.2-TC-4`). So this is no longer a spec *defect* — it is the **single remaining desktop-owned mechanics gap**, and the only blocker whose canonical artifacts do not live in the `server` repo.

**What is still missing (the "HB-1" mechanics, absent from both repos):**
- **CRDT interaction (the hard part):** SPECIFICATION §6 / §16.2a say a plaintext scope keeps "only the **minimal CRDT/sync state needed to merge**," but the entire text-sync model (`08`, `09`) assumes the local source of truth is the SQLCipher DB + encrypted blobs + in-memory Yrs doc. How a plaintext-on-disk working copy (which *replaces* the local copy) round-trips through the Yrs/CRDT relay path, and what "minimal CRDT/sync state" is persisted, is specified nowhere.
- **FTS interaction:** is a plaintext-on-disk file still indexed in the encrypted FTS5 store? Unspecified (`11`, `04 §4.3` assume DB-resident plaintext only).
- **Data model:** `specification/04` defines `KeepOnDevice`/`KeepOnDeviceDefault` (and mentions the reader-group cascade) but the **exact `PlaintextAtRest` tri-state column** on `FileEntity`/`FolderEntity`/`ProjectEntity` — and its **precedence vs the two existing cascades** (keep-on-device, reader-group attachment) — is not pinned to entity fields.
...
**What I'd need:** a desktop `specification/` section elaborating `16 §16.2a` into concrete mechanics — the plaintext working-copy ↔ Yrs/CRDT minimal-sync-state round-trip, FTS handling, the `PlaintextAtRest` entity column + its cascade precedence vs keep-on-device and reader-group, the integrity-check-and-restore flow, and the Settings/Storage toggle UI.
...
- **SQLCipher `bundle_e_sqlcipher` FTS5 + arch coverage** (`02 §2.5`, `18 §18.8`), **DPAPI roaming / Secret Service headless fallback UX** (`07 §7.2`, `19 §19.2`), **FTS5 at full-corpus scale** (`19 §19.6`), **NativeAOT vs single-file** (`19 §19.7`), and **hybrid-PQC library selection** (X25519+ML-KEM-768 / Ed25519+ML-DSA-65) behind `CryptoEngine` — each has a recommended resolution and is an early-validation spike, not an information gap.
...
The desktop spec is a strong, buildable blueprint for ~80% of the client. Full implementation of **all** phases is **not** possible from these two repos alone because of six hard blockers: one is the **desktop-owned** plaintext-at-rest gap (the earlier spec contradiction is now fixed by the ratified `16 §16.2a` cascade, but its implementation mechanics — CRDT round-trip, FTS handling, entity column + cascade precedence, integrity-restore, toggle UI — remain undefined; the single most important gap to close, and the only one desktop can close itself), and five are **cross-repo seams** whose canonical artifacts live in the `server` repo (itself spec-only, so the shared packages do not exist as code yet) or in still-open co-design items: shared package APIs, server REST/hub contracts, the ink format, native passkey mechanics, and key-transparency proof formats. The remaining open items — including the hybrid-PQC library selection — are spikes or ambiguities with recommended resolutions and can be built against.
